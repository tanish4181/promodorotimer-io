class ModernPomodoroOptions {
  ...
  bindEventListeners() {
    ...
    const toggleInputs = [
      ...
      this.elements.nextSessionInfo, this.elements.focusIndicator,
    ...
    if (this.elements.hideDistractions) {
        this.elements.hideDistractions.addEventListener('change', () => {
            this.updateYouTubeToggleStates();
            this.saveSettings();
        });
    }
    ...
    updateAllToggleStates() {
        ...
        breakSubToggles.forEach(toggle => {
            if (toggle) {
                ...
                toggle.closest('.toggle-setting, .setting-group')?.classList.toggle('disabled', !enforceBreaks);
            }
        });
        ...
        breakBlockingToggles.forEach(toggle => {
            if (toggle) {
                ...
                toggle.closest('.toggle-setting, .setting-group')?.classList.toggle('disabled', shouldBeDisabled);
            }
        });
    }
    
    updateYouTubeToggleStates() {
        const masterSwitch = this.elements.hideDistractions;
        ...
        subToggles.forEach(toggle => {
            if (toggle) {
                ...
                const settingGroup = toggle.closest('.setting-group');
                if (settingGroup) {
                    settingGroup.classList.toggle('disabled', !isEnabled);
                }
            }
        });
    }
  ...
  async resetDefaults() {
    ...
    try {
      ...
      if (this.backgroundAvailable) {
        await this.sendMessageToBackground({ type: "SETTINGS_UPDATED", settings: defaultSettings });
      }
